...
import { optOutPlayerStatsHighlights } from '@/lib/fan-alert-unsubscribe'
...
export const metadata = {
  title: 'Unsubscribe — stats highlight alerts',
  robots: 'noindex',
}

export default async function StatsHighlightsUnsubscribePage({ searchParams }: PageProps) {
  ...
  if (!raw) {
    return (
      <UnsubscribeShell>
        <h1 style={headingStyle}>Stats highlight emails</h1>
        <p style={bodyStyle}>
          This page opens from the unsubscribe link in a final-game stats highlight email.
        </p>
      </UnsubscribeShell>
    )
  }
  ...
  if (!parsed || parsed.scope !== 'stats_highlight') {
    return (
      <UnsubscribeShell>
        <h1 style={headingStyle}>Link not valid</h1>
        <p style={bodyStyle}>Use the link from your most recent stats highlight email.</p>
      </UnsubscribeShell>
    )
  }

  const result = await optOutPlayerStatsHighlights(supabaseAdmin, parsed.entityId)
  ...
  return (
    <UnsubscribeShell>
      ...
      <p style={bodyStyle}>
        You will not receive automated <strong>stats highlight</strong> emails from MyLeaguePortal
        for this roster entry.
      </p>
      <p style={{ ...bodyStyle, fontSize: '14px', color: 'var(--text-muted, #666)' }}>
        Game reminders and league news alerts are separate — use the unsubscribe link in those emails
