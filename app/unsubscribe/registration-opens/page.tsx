...
import { optOutPlayerRegistrationOpens } from '@/lib/fan-alert-unsubscribe'
...
export const metadata = {
  title: 'Unsubscribe — registration alerts',
  robots: 'noindex',
}

export default async function RegistrationOpensUnsubscribePage({ searchParams }: PageProps) {
  ...
  if (!raw) {
    return (
      <UnsubscribeShell>
        <h1 style={headingStyle}>Registration alert emails</h1>
        <p style={bodyStyle}>
          This page opens from the unsubscribe link in a registration alert email.
        </p>
      </UnsubscribeShell>
    )
  }
  ...
  if (!parsed || parsed.scope !== 'registration_opens') {
    return (
      <UnsubscribeShell>
        <h1 style={headingStyle}>Link not valid</h1>
        <p style={bodyStyle}>Use the link from your most recent registration alert email.</p>
      </UnsubscribeShell>
    )
  }

  const result = await optOutPlayerRegistrationOpens(supabaseAdmin, parsed.entityId)
  ...
  return (
    <UnsubscribeShell>
      ...
      <p style={bodyStyle}>
        You will not receive automated <strong>registration opens</strong> emails from MyLeaguePortal
        for this roster entry.
      </p>
      <p style={{ ...bodyStyle, fontSize: '14px', color: 'var(--text-muted, #666)' }}>
        Game reminders and drop-in reminders are separate — use the unsubscribe link in those emails
        if you want those off too.
      </p>
    </UnsubscribeShell>
  )
}
