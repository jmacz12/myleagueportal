...
- **Team directory** → **public team pages** at `**/league/[slug]/teams/[teamId]`** (identity, season, **roster** without contact info, jersey poll link when open). **Roster on public** for all plans (names, jersey #, positions)—**no emails/phones on public** by default; plain roster (no stat strips).
...
- **Team jersey poll:** **Shipped (MVP backend/public flow)** and moving UI to team detail. Public poll path stays `**/join/[slug]/jersey-poll/[pollId]`**; players submit **email** (must match season registration) + **preferred 0–99**; final jersey **#** remains organizer-assigned on **Players** (per-season uniqueness). **Next implementation (team page):** organizer button **Request jersey numbers** opens a modal to start/close request and copy link, with response/conflict review in team context instead of Dashboard Teams list.
