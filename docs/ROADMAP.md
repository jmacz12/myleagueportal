...
- **iPhone / “true fullscreen” + score overlay (web constraint):** Safari does **not** allow a parent-page **custom HTML overlay** (live scores) on top of an embedded YouTube/Twitch player in **native video fullscreen**—fullscreen is owned inside the iframe. **Product stance (fan UI):** Rely on the **embed’s own fullscreen** for the largest picture; show a short **legend** under the player that scores only appear on-page and that **all iPhone browsers use WebKit** (switching to “Chrome” on iPhone does not change behavior); **desktop** or **Android Chrome** is the best mix of big player + score strip. For **broadcast-style** scores burned into the video in true fullscreen, organizers use **OBS** (or similar) or **first-party streaming** when it ships (**Enterprise** / Phase 7).
...
| **Public league — Stream & Schedule UX** | Done | **`StreamWithOverlay`**: embed **fullscreen** only (no site-owned fullscreen button); **legend** under player explains score strip vs player fullscreen and **iPhone WebKit** (all in-app browsers). **Stream tab**: plain intro copy; desktop layout **`maxWidth: min(960px)`** so title + copy align with the **16:9** player. **Schedule tab**: entire schedule card navigates (**scoreboard** / **drop-ins**); recurring row **Show more dates** does not trigger navigation. **`GET /api/join/[slug]/sessions`**: exposes **`max_waitlist`** on drop-in **`scheduleItems`**; **`dropinSignupSummary`** shows **Roster full**, **waitlist closed**, etc. **Demo:** **`/games/demo/scoreboard`** (static preview; real games use **`/games/[id]/scoreboard`**). |
...
- **2026-05-11:** **Auth — Facebook sign-in:** Enabled **Facebook** alongside **Google** in **Clerk** (Dashboard → **Social connections**). Sign-in UI unchanged (`**app/(auth)/sign-in/[[...rest]]/page.tsx`**). **Roadmap:** **Pro → Video / stream** documents **iPhone / embed fullscreen** limits and fan-facing **legend** (player fullscreen vs on-page score strip).
- **2026-05-11:** **Stream embed UX — fullscreen legend only:** **`StreamWithOverlay`** — removed site **“Full screen with overlay”** / immersive / orientation-lock paths; fans use the **provider player’s fullscreen**. Added bordered **note** under the player (score strip on-page only; **iPhone** = WebKit for all in-app browsers; **desktop / Android Chrome** for best combined experience). **`app/league/[slug]/page.tsx`** Stream tab intro aligned. **Roadmap** Pro bullet + Phase 4 table row updated.
- **2026-05-11:** **Public league — Stream & Schedule polish:** **`StreamWithOverlay`** — removed transparent **bottom-half** layer that blocked embed **play/pause**; overlay score band (**pointer-events: none**). **`app/league/[slug]/page.tsx`** — Stream tab intro matches video column width on desktop (**`maxWidth: min(960px)`**, centered); Schedule cards are **whole-row links** to scoreboard or drop-ins (**Enter/Space** + click); recurring **Show more dates** uses **`stopPropagation`**. **`GET /api/join/[slug]/sessions`** — **`max_waitlist`** on drop-in schedule rows; **`dropinSignupSummary`** reads roster + waitlist caps (**Full — waitlist closed**, **Roster full**, etc.). **`/games/demo/scoreboard`** — static demo scoreboard (**`gameId === demo`** in **`app/games/[gameId]/scoreboard/page.tsx`** + **`demo-scoreboard.tsx`**).
