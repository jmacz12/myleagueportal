...
- **Join (`/join/[slug]`):** Root **`/join/[slug]`** redirects to **`/league/[slug]`** (browse lands on league home). **Season registration**, **drop-ins**, and **jersey polls** use **`/join/[slug]/register`**, **`/join/[slug]/dropins`**, **`/join/[slug]/jersey-poll/[pollId]`** respectively. Guests where configured for drop-ins; season signup one player per account; **no jersey # on season signup**.
...
| **Stripe billing & plan sync** | Done / verify ops | **Checkout** + **post-return sync** + **webhooks**: **`POST /api/stripe/sync-checkout`** (owner, session verify, idempotent org update); success URL includes **`session_id`**; webhook prefers **`organization_id`** on **`checkout.session.completed`**; **`customer.subscription.updated`** uses **Price id → plan** when metadata is empty; **`lib/stripe-org-billing.ts`** shared helpers. **Verify in Stripe Dashboard:** webhook URL for **`/api/webhooks/stripe`** in the **same test/live mode** as the secret key (API list can be empty in the opposite mode). |


### Suggested next focus (near term)

1. **Tier integrity (launch):** Audit **API + public** paths so **Basic / Pro / Enterprise** gates match product (CMS save/upload, brand limits, team logo, waivers, etc.)—clear upgrade incentive, fewer support surprises.
2. **Jersey poll 1.0:** **Confirm and apply** conflict resolution (one-click assign numbers from poll results).
3. **Sport templates:** **Basketball vs Soccer** (etc.) at **onboarding** / settings → correct **positions** for registration and lineups.
...
- **2026-05-12:** **ROADMAP:** Phase 4 adds **Stripe billing & plan sync** row; **Phase 1–3** billing bullet expanded (sync-checkout, verify script, webhook ops note); **Suggested next focus** reordered—**billing path** treated shipped; next emphasis **tier integrity**, **jersey poll apply**, **sport templates**.
